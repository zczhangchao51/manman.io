...
鉴于传统的发布方式已经不适应各种复杂的发布要求，我在公司的同事发发写的前端发布脚本的雏形上进行了改良，意在提供一个全方面覆盖并且拓展性强的命令行发布工具。
...
* 若为同时发布多个不同环境，fupublish能自动根据配置文件判断出哪些文件已构建，哪些环境已上传等，智能化的忽略重复流程；
...
事实上，fjpublish是一个由核心实例加上各种中间件函数组成的，类似于'express'的概念，以上的功能部分是由我编写的中间件完成，所以，理论上功能的多少，完全取决于搭配的中间件。
